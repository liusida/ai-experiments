...
**Model loader:** after you **Watch** a file, use the **right side of the top bar**: enter a Hugging Face **repo id** (comma- or newline-separated for several), click **Load**, then pick a checkpoint in the **dropdown** (it lists everything **in memory** for this Stonesoup process). Weights live in a **shared pool**: **Load** adds a refcount and binds names in the watched file’s kernel; switching to another experiment and loading the **same** repo reuses tensors. **Unload** removes that checkpoint from **every** cached experiment kernel (frees GPU when nothing references it); **All** clears **all** UI-managed models globally the same way. Stonesoup picks an appropriate **Auto** class (causal LM vs vision-language such as **Qwen3-VL**). Each load also creates an internal variable name (derived from the repo id); in cells you usually pass the same **repo id** to `stonesoup.load_model`. A **single** load in a batch also sets convenience globals `model`, `tokenizer`, `processor`, and `MODEL_REPO_ID`. For the HTTP API you may pass a per-item `name` to choose the internal variable. JSON field `model_kind`: `auto` (default), `causal_lm`, or `image_text`.

**Bottom dock:** the **`>_`** and **`{ }`** controls share one bottom area—**only one** of **Console** (server log) or **Variables** is expanded at a time (opening one closes the other). While **Load** is in progress, **stderr/stdout** from the backend (including **tqdm**) streams over the WebSocket; the UI folds `\r` like cell streams. Expand/collapse preferences are in **`localStorage`** (`stonesoup_console_expanded`, `stonesoup_kernel_vars_expanded`). If both were saved open, **Console** wins on load.

**In experiment cells:** with the editable `stonesoup` package installed, use `import stonesoup` then `model, processor = stonesoup.load_model("Qwen/Qwen3-VL-8B-Instruct")` using the **same repo id** as in the UI. That **reuses** weights already in the shared pool (no second copy); the cell only adds this script’s namespace binding. Outside a running Stonesoup cell, `load_model` raises `RuntimeError`. For rich HTML output, call `stonesoup.html()` (or print the render hint) then `stonesoup.display(df)` for pandas DataFrames / `_repr_html_` objects; use `stonesoup.show()` for matplotlib figures.
...
| GET | `/api/models` | `loaded`: bindings in the **current** watched kernel `[{name, repo_id}]`. `loaded_globally`: deduplicated in-memory checkpoints for the process `[{pool_key_b64, repo_id, model_kind?, …}]` (toolbar dropdown). |
| POST | `/api/models/load` | Body: `items` — list of `{repo_id, name?, model_kind?}`; optional top-level `model_kind` (`auto` default, or `causal_lm`, `image_text`); optional `device_map`, `torch_dtype`, `trust_remote_code`. `auto` picks causal LM vs vision-language from the checkpoint config (e.g. Qwen3-VL). Responds immediately with `{ok, accepted: true}`; loading continues in the background. **Cell runs and load/unload share one lock per watched file**, so **executing a cell still waits** until any in-progress load on that kernel finishes (same process mutates one namespace — they cannot overlap safely). Loads add to the **shared pool** (reused across cached kernels with the same options). |
| POST | `/api/models/unload` | `{"pool_keys_b64": ["…"]}` — remove that checkpoint from **every** cached kernel (frees GPU when refcounts hit zero). `{"names": null}` — unload **all** Stonesoup-managed models from **every** cached kernel. Optional legacy `{"names": ["binding"]}` drops that binding name from each kernel that has it. |
