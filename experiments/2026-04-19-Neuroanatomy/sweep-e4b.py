...
def _is_gemma4_per_layer_text_stack(owner: nn.Module) -> bool:
    """Gemma 4 text stack uses fused per-layer embeddings sized to ``num_hidden_layers`` at init."""
    return bool(getattr(owner, "hidden_size_per_layer_input", 0)) and hasattr(
        owner, "embed_tokens_per_layer"
    )


def _gemma4_text_forward_relayer(
    ...
    **kwargs,
    ...
    """``Gemma4TextModel.forward`` with relayer: keep ``num_hidden_layers`` for embeddings, gather per-layer rows."""
    ...
    if use_cache and past_key_values is None:
        cfg_c = deepcopy(text_model.config)
        cfg_c.num_hidden_layers = dup._new_num_layers
        lt = getattr(cfg_c, "layer_types", None)
        if lt is not None and len(lt) == dup._original_num_layers:
            cfg_c.layer_types = [lt[j] for j in dup.layer_indices]
        past_key_values = DynamicCache(config=cfg_c)
    ...
    for layer_type in text_model.unique_layer_types:
        position_embeddings[layer_type] = text_model.rotary_emb(
            hidden_states, position_ids, layer_type
        )

    shared_kv_states: dict = {}
    lt_cfg = text_model.config.layer_types
    for k in range(dup._new_num_layers):
        logical = dup.layer_indices[k]
        decoder_layer = text_model.layers[k]
        pl = (
            per_layer_inputs[:, :, logical, :]
...
class LayerDuplicatedModel(nn.Module):
    ...
    def __init__(self, base_model: nn.Module, layer_indices: list[int]):
        ...
        self._gemma4_mode = _is_gemma4_per_layer_text_stack(self._layers_owner)
        # Gemma4 KV-sharing consumers look up shared_kv_states by
        # ``kv_shared_layer_index`` (original-space), but producers store under
        # ``self.layer_idx`` which shallow-copy remapped to new-space. Rewrite
        # the consumer's index to the most-recent new position of the producer.
        self._remap_kv_shared_indices()

    def _remap_kv_shared_indices(self) -> None:
        for new_pos, orig_i in enumerate(self.layer_indices):
            attn = getattr(self._new_layers[new_pos], "self_attn", None)
            if attn is None or not getattr(attn, "is_kv_shared_layer", False):
                continue
            orig_attn = getattr(self._original_layers[orig_i], "self_attn", None)
            orig_target = getattr(orig_attn, "kv_shared_layer_index", None) if orig_attn is not None else None
            if orig_target is None:
                continue
            remapped = next(
                (k for k in range(new_pos - 1, -1, -1) if self.layer_indices[k] == orig_target),
                orig_target,
            )
            attn.kv_shared_layer_index = remapped

    @contextmanager
    def _swap_layers_only(self):
        """Replace decoder ``ModuleList`` only (no ``num_hidden_layers`` / embedding shape changes)."""
    ...
    def _apply_layer_config(self):
        """Patch layer list + config counts (dense LMs). Gemma 4 per-layer embeds use :meth:`_swap_layers_only` + custom forward."""
        owner = self._layers_owner
        attr = self._layers_attr
        orig_layers = getattr(owner, attr)
    ...
    @contextmanager
    def _gemma4_forward_patch(self):
        owner = self._layers_owner
        orig_forward = owner.forward

        def _relayer_forward(*args, **kwargs):
            return _gemma4_text_forward_relayer(self, owner, *args, **kwargs)

        try:
            owner.forward = _relayer_forward  # type: ignore[method-assign]
            yield
        finally:
            owner.forward = orig_forward

    def forward(self, *args, **kwargs):
        if not self._gemma4_mode and kwargs.get("past_key_values") is not None:
        ...
        if self._gemma4_mode:
            with self._swap_layers_only(), self._gemma4_forward_patch():
    ...
    def generate(self, *args, **kwargs):
        ...
        if self._gemma4_mode:
            # Gemma4 reshapes per_layer_inputs by config.num_hidden_layers, so we
            # keep the original count in config. generate() would otherwise auto-
            # build DynamicCache with the original slot count, and layers with
            # new_layer_idx >= original_num_layers crash on cache.update().
            cfg_c = deepcopy(self.base_model.config.get_text_config(decoder=True))
            cfg_c.num_hidden_layers = self._new_num_layers
            lt = getattr(cfg_c, "layer_types", None)
            if lt is not None and len(lt) == self._original_num_layers:
                cfg_c.layer_types = [lt[j] for j in self.layer_indices]
            kwargs["past_key_values"] = DynamicCache(config=cfg_c)
            with self._swap_layers_only(), self._gemma4_forward_patch():
...
if _baseline_score == _baseline_score:
    ...
    _heatmap_title = f"{MODEL_ID} − math_16 mean score − baseline (0,0); similarity clipped [0,1]"
...
_layer_indices_b0 = expand_single_block(_N, (0, 0))
_dup_b0 = build_model_with_layers(model, _layer_indices_b0)
_dup_b0.eval()
_pad_b0 = int(tokenizer.pad_token_id or tokenizer.eos_token_id)
_baseline_rows: list[dict[str, str | int]] = []
for _b0 in range(0, len(_sweep_keys_valid), SWEEP_BATCH_SIZE):
    stonesoup.check_abort()
    _chunk = _sweep_keys_valid[_b0 : _b0 + SWEEP_BATCH_SIZE]
    _row_ids: list[torch.Tensor] = []
    _row_m: list[torch.Tensor] = []
    for _key in _chunk:
        _item = _dataset_sweep[_key]
    ...
    for _ids, _am in zip(_row_ids, _row_m, strict=True):
        ...
        if _pad:
            _ids = torch.cat(
                [
                    torch.full((1, _pad), _pad_b0, dtype=_ids.dtype, device=device),
                    _ids,
                ],
                dim=1,
            )
            _am = torch.cat(
                [
                    torch.zeros((1, _pad), dtype=_am.dtype, device=device),
                    _am,
                ],
                dim=1,
            )
    ...
    _batch_attn = torch.cat(_batched_m, dim=0)
    with torch.inference_mode():
        _out_ids = _dup_b0.generate(
            input_ids=_batch_input_ids,
            attention_mask=_batch_attn,
            max_new_tokens=MAX_NEW_TOKENS,
            do_sample=False,
            pad_token_id=_pad_b0,
        )
    for _bi, _key in enumerate(_chunk):
        _ans = int(_dataset_sweep[_key]["answer"])
        _completion = tokenizer.decode(_out_ids[_bi, _maxL:], skip_special_tokens=True)
        _pred = parse_first_integer(_completion)
        _q = _dataset_sweep[_key]["question"]
        _baseline_rows.append(
            {
                ...
                "answer": _ans,
                "prediction": _pred,
                "output": " ".join(_completion.split()),
            }
        )
del _dup_b0
print(f"Baseline (i,j)=(0,0); mean_score={_mat[0, 0]!r} (NaN if not run yet)", flush=True)
for _r in _baseline_rows:
    print(
        f"\n--- key={_r['key']} score={similarity_score(int(_r['prediction']), int(_r['answer'])):.6f} ---\n"
        f"Q: {_r['question']}\n"
        f"answer: {_r['answer']!r}  prediction: {_r['prediction']!r}\n"
        f"output: {_r['output']!r}",
        flush=True,
    )
